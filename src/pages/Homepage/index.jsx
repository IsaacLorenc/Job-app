...
const Homepage = () => {
  ...
  const updateObservedNodes = useCallback((nodeToRemove) => {
    setObservedNodes((observedNodes) =>
      observedNodes.filter((node) => node !== nodeToRemove)
    );
  }, [setObservedNodes]);

  useEffect(() => {
    if (targets.current.size > 0) {
      setObservedNodes([...targets.current]); // Update only if there are new targets
    }
  }, [targets.current]); // Depend only on `targets.current` changing
  ...
  useEffect(() => {
    for (const entry of entries) {
      if (entry.isIntersecting) {
        ...
        setObservedNodes((observedNodes) => {
          if (observedNodes.includes(entry.target)) {
            return observedNodes.filter((node) => node !== entry.target);
          }
